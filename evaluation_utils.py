...
width_to_focal[2040] = 1366.2954658193316

def load_gt_disp_kitti(path):
    gt_disparities = []
    for i in range(371):
        disp = cv2.imread(path + "depth_map_" + str(i).zfill(6) + ".png", -1)
        disp = disp[:,:,0]
        disp.shape
...
def convert_disps_to_depths_kitti(gt_disparities, pred_disparities):
    ...
    for i in range(len(gt_disparities)):
        ...
        pred_nom = np.median(pred_disp)
        gt_nom = np.median(gt_disp)
        pred_disp = (gt_nom/gt_nom)*pred_disp
